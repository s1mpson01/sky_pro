...
def mask_account_card(account_card_number: str) -> str:
    ...
    if type_account in ["Счет "]:
        mask_account_card_number = type_account + get_mask_account(number_account)
    else:
        mask_account_card_number = type_account + get_mask_card_number(number_account)
...
def get_date(data_string: str) -> str:
    ...
    data_string_search = re.search(r"^\d{4}-\d{2}-\d{2}", data_string)

    if isinstance(data_string_search, re.Match):

        if (
            int(data_string[8:10]) in range(1, 32)
            and int(data_string[5:7]) in range(1, 13)
            and int(data_string[:4]) > 0
        ):
